...
public class BigQueryCommonSteps {
  ...
  public void getCountOfNoOfRecordsTransferredToTargetBigQueryTable() throws IOException, InterruptedException {
    ...
    Assert.assertEquals("Number of records transferred to BigQuery should be equal to " +
                          "records out count displayed on the Source plugin: ", countRecords,
                        CdfPipelineRunAction.getCountDisplayedOnSourcePluginAsRecordsOut());
    Assert.assertEquals("Number of records transferred to BigQuery should be equal to " +
                          "records in count displayed on the Sink plugin: ", countRecords,
                        CdfPipelineRunAction.getCountDisplayedOnSinkPluginAsRecordsIn());
  }
